def create_parser():
    ...
    desc = (
        "Create snapshots from segmentation maps. \n\nData can be "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If True, bounding box will be fixed. If False, adjusted "
        " for each slice.",
    )
    parser.add_argument(
        "--opacity",
    ...
    parser.add_argument(
        ...
        " will be rendered as an animation.",
    ...
    parser.add_argument(
        "--disable_warnings", required=False, action="store_true", default=False
    )
...
def __check_axes__(axes):
    ...
    ax_names = ("x", "y", "z")
    if (
        (isinstance(axes, str) or isinstance(axes, Iterable))
        and len(axes) > 0
        and len(axes) < 4
    ):
        for e in axes:
    ...
    msg = (
        "axes should be a single character or an Iterable with the"
...
def check_logic(args):
    ...
    if args.output.endswith(".gif") and (
        (args.config and args.nobg) or (not args.config and not args.bg)
    ):
...
def run(args):
    ...
    if args.config:
        ...
        fp = (
            args.files[0].name if len(args.files) == 1 else [e.name for e in args.files]
        )
