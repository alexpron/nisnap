...
def __preproc_aseg__(aseg_fp, rawavg_fp, cache=False):
    ...
    cmd = (
        "mri_label2vol --seg {aseg} --temp {raw} --o {aseg_T1space} "
        " --regheader {aseg}"
    )
    ans = os.system(cmd.format(aseg=aseg_fp, raw=rawavg_fp, aseg_T1space=fp))
    if ans != 0:
        msg = (
            "FreeSurfer command `mri_label2vol` failed. Please check that "
